...
| **A non-string, non-`null` `dedupByKey` key is an error `tryEval` does not catch** — it is an interpreter type error at the dynamic-attr site, not a `throw` | `ok (p.dedupByKey (n: n.k) [ { k = 1; } { k = 1; } ])` ⇒ the whole eval aborts with `error: expected a string but found an integer: 1` at `lib/default.nix:149:36`, the caret under `seen ? ${k}` — one of `go`'s two dynamic-attribute sites (the other is the `seen // { ${k} = true; }` at `152`). Controls, same run: `ok` over the same call with `k = "1"` ⇒ `true`; `ok (p.last [ ])` ⇒ `false`, i.e. `tryEval` *does* catch this lib's explicit `throw`s. ★ **Re-run at `da70563`, not re-pointed.** The transcript recorded here cited `lib/default.nix:172:36`, a coordinate that had stopped naming the dynamic-attr site — at this revision line 172 is a `groupBy` comment — so the quoted evidence pointed at code that could not have produced it. The message text and the column survive the re-run unchanged; only the line moved. A transcript asserts that something *was measured*, so the honest repair is to measure again and publish what the run emitted, never to edit a coordinate underneath a message the new location never produced |
