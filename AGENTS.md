...
| Demand-driven attribute schedules, convergence loops | `gen-scope` for the **convergence loop** — the Kleene-ascent primitive is `circular` in `gen-scope/lib/resolve.nix`. The static **attribute schedule** has landed NOWHERE: the transfer ruling names its destination, and `gen-scope.lib` exports no schedule construct at this pin. **`gen-resolve` DISSOLVED rather than moving as one library.** ADR-0008 §4 orphans the repository as reference and the 2026-09-02 transfer ruling dispositioned its eleven exports ROW BY ROW rather than as one content block (map at `den-hoag-p3y9`); take no new dependency on it |
